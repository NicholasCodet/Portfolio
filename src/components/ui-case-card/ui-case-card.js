...
export function createCaseCard({ title, description, href, imageUrl, spriteInline = true } = {}) {
  ...
  if (media) {
    const resolved = imageUrl ? (resolveAssetPath(imageUrl, import.meta.url) || imageUrl) : '';
    if (resolved) {
  ...
  if (read) {
    const icon = createSvgUse('icon-arrowRight-linear', { size: 24, className: 'icon linear' });
