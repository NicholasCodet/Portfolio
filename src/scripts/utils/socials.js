...
export function renderSocialIcon(
  s,
  { size = 20, className = "icon", ariaHidden = true } = {}
) {
...
export function renderSocialLink(
  s,
  { withLabel = false, size = 20, className } = {}
) {
  ...
  const safeHref = sanitizeHref(String(s.href || ""));
  if (!safeHref) {
    return `<span${cls}${aria}>${content}</span>`;
  }
  return `<a${cls} href="${escape(safeHref)}" target="_blank" rel="noopener noreferrer"${aria}>${content}</a>`;
}
